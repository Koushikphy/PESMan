...
def parseFile(sFile):
    ...
    with open(sFile, 'r') as f:
        for i in f:
            i = i.strip()
            if not i or i.startswith('#'): continue  # ignore for blank or commented line
            if '#' in i:                            # ignore inline comments
                i = i[:i.find('#')]
            ll = map(str.strip, i.split(':'))
            pairs[ll[0]] = ll[1]
    return pairs
...
def addNewCalcType(db, conf):
    ...
    tconf = parseFile(conf)

    with open(tconf['template']) as f:  stemp = f.read()

    # in case of any missing keys it will throw a keyerror,
    # no need to explicitly assert each and every steps.
    with sqlite3.connect(db) as con: # context manager to auto commit and roll back if error
        cur = con.cursor()
        cur.execute("""INSERT INTO CalcInfo (Type,Name,Depends,InpTempl,OrbRec,ResVars,Desc) VALUES (?, ?, ?, ?, ?, ?, ?)""", 
                    tconf["type"],tconf["name"],tconf["depends"],stemp,tconf["record"],tconf["resvars"],tconf["desc"])
