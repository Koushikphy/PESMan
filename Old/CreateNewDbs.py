   ...
   for geom in lgeom:
         ...
         curMain.execute('''select * from Geometry where
                           ...
                           (phi between ? and ?)''', (rho-4.5,rho+4.5,\
                                                      theta-misc.ToRad(30.0),theta+misc.ToRad(30.0),\
...
#==============================================================================
   ...
   lrho = [10.0]
   lthe = [(3.0 + 3.0*i)*pi/180.0 for i in range(30) ]
   lphi = [(0.0 + 3.0*i)*pi/180.0 for i in range(61) ]

   llgeom = gengrid.GenerateHSGrid(lrho,lthe,lphi)
