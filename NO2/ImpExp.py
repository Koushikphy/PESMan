...
def GetResultsNact(fres):
      ...
      str12_1 += f.readline()
      str12_1 += f.readline()
      str12_1 = str12_1.split()
      str12_1 = [i.replace('D','E') for i in str12_1]
      ...
      str12_2 += f.readline()
      str12_2 += f.readline()
      str12_2 = str12_2.split()
      str12_2 = [i.replace('D','E') for i in str12_2]
 
      f.close()

      lresults = str12_1 + str12_2
...
def ImportCalc(Db,CalcDir,CalcFile,DataDir,Verbose=False,Check=False):
   ...
   try:
      ...
      gobj = geometry.Geometry(rho=GeomRow["rho"],phi=GeomRow["phi"],id=GeomRow["id"])
      sXYZ = gobj.to_xyzstr()
...
def ExportCalc(Db,GeomId,CalcTypeId,DataDir,ExpDir,ComTemplate="",StartId=0,BaseSuffix="",Verbose=False):
   ...
   with sqlite3.connect(Db) as con:
      ...
      GeomObj = geometry.Geometry(GeomRow["rho"],GeomRow["phi"],id=GeomId)
...
def RunMolpro(RunDir,ScrDir,ComFile,bDryRun=False):
   ...
   with cd(RunDir):
      if bDryRun:
         ...
         exitcode = subprocess.call(["molpro", "-d", ScrDir, "-W .", "-n 4",ComFile])
      return exitcode
