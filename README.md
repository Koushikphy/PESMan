...
After few initial geometries are done sequentially and serially, the parallel implementation of `RunManager` can be invoked by changing the `process` value in the script to more than `1`, value `1` meaning serial execution. In that case, the script will export a bunch of jobs from the neighbourhood of successful geometries and run them in parallel.
